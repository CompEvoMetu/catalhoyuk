...
BurialData <- read_excel("CatalData.xlsx")

BurialData$mtDNA_Major <- BurialData$"mtDNA HPG"
BurialData$mtDNA_Major <- substr(BurialData$mtDNA_Major, 1, 2)

BurialData$chrY_Major <- BurialData$"chrY HPG"
BurialData$chrY_Major <- substr(BurialData$chrY_Major, 1, 2)

HaploData <- BurialData %>% select("Sample ID", mtDNA_Major, chrY_Major, Period, Building) %>%
  filter(!is.na(Building)) %>% filter(Period %in% c("Early","Middle","Late"))

HaploData_Y <- HaploData %>% select("Sample ID", chrY_Major, Period, Building) %>% 
  filter(!is.na(chrY_Major)) %>% 
  filter(nchar(chrY_Major) > 1) %>%
  rename(Haplogroup = chrY_Major)

HaploData_mt <- HaploData %>% select("Sample ID", mtDNA_Major, Period, Building) %>%
  filter(!is.na(mtDNA_Major)) %>%
  filter(nchar(mtDNA_Major) > 1) %>%
  rename(Haplogroup = mtDNA_Major)

HaploData_mt$Haplogroup[HaploData_mt$Haplogroup == "H1" | HaploData_mt$Haplogroup == "H+" | HaploData_mt$Haplogroup == "H" | HaploData_mt$Haplogroup == "H2" | HaploData_mt$Haplogroup == "H4"] <- "HV"

Pool_Y <- HaploData_Y %>%
  group_by(Building) %>%
  reframe(UniqueHaplos = unique(Haplogroup)) %>% pull("UniqueHaplos")

Pool_mt <- HaploData_mt %>%
  group_by(Building) %>%
  reframe(UniqueHaplos = unique(Haplogroup)) %>% pull("UniqueHaplos")
...
for (sim in 1:num_sims) {
  ...
  #Select 20 Buildings
  selected_buildings <- sample(unique(PairResultsWB$BUILDING1), 20)
...
OutputFileName <- paste("matrilocal",matrilocal_proportion,"patrilocal",100-matrilocal_proportion,generation,sim,"tsv",sep = ".")
