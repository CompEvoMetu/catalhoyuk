...
inds = pd.read_csv(args.burial, sep = '\t', converters = {'Building': str, 'Individual': str, 'Unit': str})

#Remove information of unmerged identical individuals
inds = inds[inds['Sample_ID'] != 'cch328']
inds = inds[inds['Sample_ID'] != 'cch320']
inds = inds[inds['Sample_ID'] != 'cch341']
inds = inds[inds['Sample_ID'] != 'cch348']

#Select and rename necessary columns
inds = inds.fillna('')
inds = inds[['Unit', 'Individual', 'Period', 'Building', 'Age_Category', 'Dataset']]
inds.loc[(inds['Age_Category'] == 'child_infant'), 'Age_Category'] = 'infant'
inds = inds.rename(columns={'Age_Category': 'Age'})
...
dataset = inds[inds['Dataset'] ==  'used']
...
#Select and rename necessary columns
res.loc[(res['Age_Category1'] == 'child_infant'), 'Age_Category1'] = 'infant'
res.loc[(res['Age_Category2'] == 'child_infant'), 'Age_Category2'] = 'infant'
res = res[(res['Period1'] == 'Early') | (res['Period1'] == 'Middle') | (res['Period1'] == 'Late')]
res = res.drop(['Age1', 'Age2'], axis = 1)
res = res.rename(columns={'Age_Category1': 'Age1', 'Age_Category2': 'Age2'})
